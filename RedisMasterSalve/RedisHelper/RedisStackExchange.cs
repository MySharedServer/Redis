...
namespace RedisHelp
{
    public class RedisStackExchange
    {
        ...
        public RedisStackExchange(int dbNum = 0)
                : this(dbNum, null)
        {
        }

        public RedisStackExchange(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
            _conn =
        ...
        #region batch
        public async Task BulkSetAsync(List<BatchDataModel> dataList)
        {
            BatchDataModel test = new BatchDataModel();
            await Task.Run(() => {
                var db = _conn.GetDatabase(DbNum);
                var batch = db.CreateBatch();
                foreach (BatchDataModel data in dataList)
                {

                    string key = AddSysCustomKey(data.Entity);
                    if (data.Expiry != default(DateTime))
                    {
                        batch.KeyExpireAsync(key, data.Expiry);
                    }

                    switch (data.Type)
                    {
                        case DataFieldType.INFORM:
                            batch.ListRightPushAsync(key + "#" + data.Name, ConvertJson(data.Value));
                            break;
                        default:
                            batch.HashSetAsync(key, data.Name, ConvertJson(data.Value));
                            break;
                    }
                }
        ...
        public T HashGetAll<T>(string key)
        {
            ...
            return Do(db =>
            {
                ...
                foreach (HashEntry item in entry)
                {
                    dic.Add(item.Name, ConvertObj<object>(item.Value));
                }
                return ConvertObj<T>(ConvertJson(dic));
            });
        }
